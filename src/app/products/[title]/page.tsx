    ...
    <div className="products-home">
      ...
      { data && data.product &&
        <div className="product-item">
          <section>
            <img src={data.product.thumbnail.url}></img>
          </section>
          <section className="single-product-details">
            <section className="top-details">
              <h1>{data.product.name}</h1>
              <p dangerouslySetInnerHTML={JSON.parse(data.product.description)?.blocks.map(
                    (item: any) => item.data.text
                  )[0]}>
              </p>
              <h5>
                <Rating page={"pdp"} />
              ...
              <h4>
                Special price
                <div className="price">
                ${parseFloat(data.product.pricing.priceRange.start.gross.amount -
                ...
                <h5>Available offers</h5>
                {
                  BANK_OFFERS.map((offer, id)=>{
                    return <li key={id}>
                      ...
                      {offer} <Link href='/' style={{color: 'green'}}>TnC</Link>
                    </li>
                  }
                )
                }
